...
public class RecordToNIST_MSP {
	...
	public static String convert(Record record) {
		...
		if (record.getMsFocusedIon().stream().anyMatch(p -> "PRECURSOR_TYPE".equals(p.key()))) {
			sb.append("Precursor_type: ")
					.append(record.getMsFocusedIon().stream()
		...
		if (record.getMsFocusedIon().stream().anyMatch(p -> "PRECURSOR_M/Z".equals(p.key()))) {
...
There is one mandatory field, namely Parent=<m/z>, which is the precursor ion m/z required for searching.
		...
		sb.append("Comments: ").append("Parent=")
				.append(record.getMsFocusedIon().stream()
					.filter(p -> "PRECURSOR_M/Z".equals(p.key()))
